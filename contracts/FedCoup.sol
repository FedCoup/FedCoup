...
contract FedCoup is MintableToken {
    ...
    */
    function createCoupons(uint _FCC) onlyPayloadSize(2 * 32) {
        ...
        *  Formula: number of B coupons = (given FCC * FCC price / constant_coupon_price ) 
        */
        ...
        *  Formula: number of S coupons = (given FCC * FCC price / constant_coupon_price ) 
        */
        ...
        if ( currentBcoupons > 0 ) {
            ...
            if ( couponDist <= 1 ) {
                calculatedBcoupons = couponDist.mul( idealBcoupons );
